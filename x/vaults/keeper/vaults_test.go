...
func (s *KeeperTestSuite) TestRepayDebt() {
	...
	var (
		denom           = "atom"
		repayAsset      = sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(2000000))
		...
		mintedCoin      = sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(300000000))
	)
	...
	tests := []struct {
	...
	}{
		{
			...
			liquidation: types.Liquidation{
				...
				LiquidatingVaults: []*types.Vault{
					{
						...
						Debt:             sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(25_000_000)),
				...
				VaultLiquidationStatus: map[uint64]*types.VaultLiquidationStatus{
					0: {
						Sold:             sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(35_000_000)),
						RemainCollateral: sdk.NewCoin("atom", math.ZeroInt()),
					},
				},
			},
			expVaultStatus:  []types.VaultStatus{types.CLOSED},
			reserveBalances: sdk.NewCoins(sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(10_000_000))),
		},
		{
			...
			liquidation: types.Liquidation{
				...
				VaultLiquidationStatus: map[uint64]*types.VaultLiquidationStatus{
					0: {
						Sold:             sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(20_000_000)),
						RemainCollateral: sdk.NewCoin("atom", math.ZeroInt()),
					},
				},
			},
			expVaultStatus:  []types.VaultStatus{types.LIQUIDATED},
			shortfallAmount: sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(5_000_000)),
		},
		{
			...
			reserveBalances: sdk.NewCoins(
				sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(1_000_000)),
		...
		},
		{
			...
			liquidation: types.Liquidation{
				...
				VaultLiquidationStatus: map[uint64]*types.VaultLiquidationStatus{
					0: {
						Sold:             sdk.NewCoin(types.DefaultMintDenom[0], math.ZeroInt()),
		...
		},
		{
			...
			liquidation: types.Liquidation{
				...
				VaultLiquidationStatus: map[uint64]*types.VaultLiquidationStatus{
					0: {
						Sold:             sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(7_000_000)),
			...
			shortfallAmount: sdk.NewCoin(types.DefaultMintDenom[0], math.NewInt(18_000_000)),
		},
	}
	for _, t := range tests {
		s.Run(t.name, func() {
			...
			err = s.App.VaultsKeeper.Liquidate(s.Ctx, t.liquidation, types.DefaultMintDenom[0])
			s.Require().NoError(err)
