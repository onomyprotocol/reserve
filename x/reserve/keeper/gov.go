...
func (k Keeper) HandleCreateGaugeProposal(ctx sdk.Context, p *types.CreateDenomProposal) error {
	for _, group := range p.CreateGroups {
		// reserveModuleAddress := k.ak.GetModuleAddress(types.ModuleName)
...
func NewDenomProposalHandler(k Keeper) govtypesv1.Handler {
	return func(ctx sdk.Context, content govtypesv1.Content) error {
		switch c := content.(type) {
		case *types.CreateDenomProposal:
			return k.HandleCreateDenomProposal(ctx, c)
