...
func (k Keeper) SetInitPrice(ctx context.Context) {
	initPrice := []*types.BandPriceState{
		// multiplier = 1000
		{
			Symbol:      vaultstypes.DefaultMintDenom,
			Rate:        math.NewInt(1000), //1
			ResolveTime: uint64(time.Now().Unix()),
			PriceState:  *types.NewPriceState(math.LegacyMustNewDecFromStr("1"), time.Now().Unix()),
		},
		{
			Symbol:      "anom",
			Rate:        math.NewInt(30), //0.03
			ResolveTime: uint64(time.Now().Unix()),
			PriceState:  *types.NewPriceState(math.LegacyMustNewDecFromStr("0.03"), time.Now().Unix()),
		},
		{
			Symbol:      "uatom",
			Rate:        math.NewInt(8000), //8
			ResolveTime: uint64(time.Now().Unix()),
			PriceState:  *types.NewPriceState(math.LegacyMustNewDecFromStr("8"), time.Now().Unix()),
		},
		{
			Symbol:      "uosmo",
			Rate:        math.NewInt(450), //0.45
			ResolveTime: uint64(time.Now().Unix()),
			PriceState:  *types.NewPriceState(math.LegacyMustNewDecFromStr("0.45"), time.Now().Unix()),
		},
		{
			Symbol:      "uusdt",
			Rate:        math.NewInt(1000), //0.45
			ResolveTime: uint64(time.Now().Unix()),
			PriceState:  *types.NewPriceState(math.LegacyMustNewDecFromStr("1"), time.Now().Unix()),
		},
		{
			Symbol:      "atom",
			Rate:        math.NewInt(8000), //0.45
			ResolveTime: uint64(time.Now().Unix()),
			PriceState:  *types.NewPriceState(math.LegacyMustNewDecFromStr("8"), time.Now().Unix()),
		},
		{
	...
	for _, i := range initPrice {
		k.SetBandPriceState(ctx, i.Symbol, i)
	}
