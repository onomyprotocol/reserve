...
func (k Keeper) stablecoinUpdate(ctx context.Context, newPrice math.LegacyDec, stablecoin types.Stablecoin) types.Stablecoin {
	...
	// fee in anf out > 0
	// newFeeIn = feeIn - k_in * deltaP
	// newFeeOut= feeOut + k_out * deltaP
	// deltaP > 0 then fee_in will decrease, otherwise it will increase
	feeIn := math.LegacyMaxDec(stablecoin.FeeIn.Sub(params.AdjustmentFeeIn.Mul(deltaP)), math.LegacyZeroDec())
	feeOut := math.LegacyMaxDec(stablecoin.FeeOut.Add(params.AdjustmentFeeOut.Mul(deltaP)), math.LegacyZeroDec())
	...
	stablecoin.FeeIn = math.LegacyMinDec(feeIn, math.LegacyMustNewDecFromStr(feeMax))
	stablecoin.FeeOut = math.LegacyMinDec(feeOut, math.LegacyMustNewDecFromStr(feeMax))
